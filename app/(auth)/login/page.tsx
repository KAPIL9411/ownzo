...
export default function LoginPage() {
  ...
  // Already authenticated → redirect (global overlay in Providers covers this too)
  useEffect(() => {
    if (isAuthenticated) {
      router.replace('/')
    }
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      // 'success'     → global overlay is now showing, onAuthStateChanged will redirect
      // 'cancelled'   → user closed popup, reset button
      // 'redirecting' → full page redirect in progress, do nothing
      if (result === 'cancelled') {
        setLoading(false)
      }
      // For 'success' and 'redirecting', keep loading=true so button doesn't snap back
    } catch {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        ...
        {/* Decorative arc */}
        <svg className="absolute bottom-0 right-0 opacity-20 pointer-events-none" width="320" height="320" viewBox="0 0 320 320" fill="none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 bg-white">
        <div className="w-full max-w-[400px]">
          ...
          <div className="lg:hidden flex items-center gap-2.5 mb-10">
            <div className="h-9 w-9 rounded-xl flex items-center justify-center" style={{ background: '#0D1F17' }}>
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-400 font-medium">Sign in to your marketplace account</p>
          </div>
            ...
            <span className="relative flex items-center gap-3 w-full px-5">
              {loading ? (
                <>
                  ...
                  <span className="text-gray-500">Signing you in…</span>
                </>
              ) : (
                <>
          ...
          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="w-full h-[52px] flex items-center justify-center gap-2 rounded-2xl text-white text-sm font-bold transition-all duration-200 active:scale-[0.98] disabled:opacity-50"
